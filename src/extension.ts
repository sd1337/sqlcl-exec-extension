...
const PlSqlParser = require('./parser/PlSqlParser').PlSqlParser;
...
let outputChannel = vscode.window.createOutputChannel("sqlcl execute");
...
export function activate(context: vscode.ExtensionContext) {
	...
	if(config.get("sqlclExecute.ordsServices")){
		const nodeDependenciesProvider = new OrdsTreeProvider(rootPath, connection, extensionName);
		// vscode.window.registerTreeDataProvider('apexApplications', nodeDependenciesProvider);
		vscode.window.createTreeView('ordsServices', {
		treeDataProvider: nodeDependenciesProvider
		});
	}
	let disposable = vscode.commands.registerCommand(`${extensionName}.helloWorld`, () => {
	});
